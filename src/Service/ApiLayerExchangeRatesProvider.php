...
class ApiLayerExchangeRatesProvider implements ExchangeRatesProviderInterface
{
    private const RATES = 'rates';
    private const API_KEY = 'apikey';
    private const EXCHANGE_SITE_VAR_KEY = 'EXCHANGE_PROVIDER_URL';
    ...
    public function get(string $currency): float
    {
        ...
        if (empty($exchangeUrl)) {
            throw InternalServerErrorException::createFromMessage(\sprintf('Missing ENV variable %s!', self::EXCHANGE_SITE_VAR_KEY));
        }
